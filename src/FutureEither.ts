...
export class FutureEither<L, R> {
  ...
  // map<U>(f: (r: R) => Future<U>): FutureEither<L, U>
  // map<U>(f: (r: R) => Either<L, U>): FutureEither<L, U>
  // map<U>(f: Function): unknown {
  //   return FutureEither.of(
  //     this.fe.flatMap(e => e.match({
  //       Left: l => Future.ok(Either.left<L, U>(l)),
  //       Right: r => {
  //         const result = f(r)
  //         console.log(typeof result)

  //         if (result instanceof Future) return result.map(u => Either.right<L, U>(u))
  //         if (result instanceof Left || result instanceof Right) return Future.ok(result as Either<L, U>)
  //         return Future.ok(Either.right<L, U>(result as U))
  ...
  //     this.fe.flatMap(e => e.match({
  //       Left: l => Future.ok(Either.left<L, U>(l)),
  //       Right: r => f(r).map(u => Either.right<L, U>(u))
  //     }))
  //   )
  // }
  ...
  flatMap<L2, U>(f: (r: R) => FutureEither<L2, U> | Future<Either<L2, U>>): FutureEither<L | L2, U> {
    return FutureEither.of(this.fe.flatMap(e => e.match({
      Left: l => Future.ok(Either.left<L | L2, U>(l)),
      Right: r => {
        const result = f(r)
  ...
  leftWiden<L2>(): FutureEither<L | L2, R> {
    return this as FutureEither<L | L2, R>
  }
